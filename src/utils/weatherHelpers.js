...
export function getEncouragementMessage(tempCelsius, main = '') {
  ...
  if (isSnowy) {
    return {
      emoji: '❄️',
      message:
        '눈 내리는 오늘, 길이 미끄러울 수 있으니 천천히 다녀오세요. 따뜻하게 챙겨입는 것 잊지 마세요!',
    }
  }
  if (isRainy) {
    return {
      emoji: '☔',
      message: '비 소식이 있어요. 우산 챙기고, 발걸음 조심히 옮기는 하루 되세요.',
    }
  }
  if (tempCelsius >= 33) {
    return {
      emoji: '🥵',
      message: '무더위가 절정이에요. 수분 보충 자주 하시고 그늘에서 잠깐씩 쉬어가세요.',
    }
  }
  if (tempCelsius >= 28) {
    return {
      emoji: '☀️',
      message: '더운 하루가 예상돼요. 시원한 물 한잔과 함께 활기차게 시작해봐요!',
    }
  }
  if (tempCelsius >= 18) {
    return {
      emoji: '🌤️',
      message: '딱 좋은 날씨네요. 오늘 하루도 산뜻하게, 좋은 일만 가득하길 바라요!',
    }
  }
  if (tempCelsius >= 10) {
    return {
      emoji: '🍂',
      message: '선선한 공기가 기분 좋은 하루예요. 가벼운 겉옷 하나 챙기면 딱 좋겠어요.',
    }
  }
  if (tempCelsius >= 0) {
    return {
      emoji: '🧣',
      message: '쌀쌀한 날씨예요. 목도리 하나 두르고 따뜻하게 하루를 시작해봐요.',
    }
  }
  return {
    emoji: '🥶',
    message: '매서운 추위가 이어져요. 실내에서도 온기를 잃지 않도록 든든히 챙겨입으세요.',
  }
}
